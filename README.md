...
Agent Pipeline 是执行体：负责解析 DOCX、处理盲审意见、生成修改方案、输出报告。
```
...
当前 `full` 是稳定的确定性流程。深度 planner/auditor agent 闭环已经定义在 `agent_specs/` 和 `.claude/agents/` 中，适合由 Claude Code、Codex 或其他宿主 Agent 调度。
...
- `full` 当前主要是确定性流程；深度 planner/auditor 闭环需要宿主 Agent 调度。
